...
export const calcBondDetails = createAsyncThunk(
  ...
  async (
    { bond, value, provider, networkID, address }: ICalcBondDetailsAsyncThunk,
    { dispatch },
    ...
    try {
      ...
      if (bond.name === "cvx") {
        ...
        let bondPriceRaw = await bondContract.bondPrice();
        bondPrice = ethers.utils.parseUnits(bondPriceRaw.toString(), 16);
...
export const bondAsset = createAsyncThunk(
  ...
  async ({ value, address, bond, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    ...
    const reserveContract = bond.getContractForReserve(networkID, signer);
    const decimal = Number(BigNumber.from(await reserveContract.decimals()).toString());
    const valueInWei = ethers.utils.parseUnits(value.toString(), decimal);
    ...
    try {
      ...
      ///
      ///
      // const akeContract = new ethers.Contract("0x2542250239e4800B89e47A813cD2B478822b2385", ierc20ABI, signer);

      // let approveTx;
      // approveTx = await akeContract.approve(
      //   "0xAafBb6eDbdF4D8eF0bAA7f898a359f1edb23d9eC",
...
const setLptokenState = (state: IBondSlice, payload: any) => {
...
const bondingSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(calcTokenDetails.pending, state => {
        state.loading = true;
      })
      .addCase(calcTokenDetails.fulfilled, (state, action) => {
        setLptokenState(state, action.payload);
        state.loading = false;
      })
      .addCase(calcTokenDetails.rejected, (state, { error }) => {
        state.loading = false;
        console.error(error.message);
      });
  },
