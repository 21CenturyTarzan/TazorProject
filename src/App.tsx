...
import { Stake, ChooseBond, Bond, TreasuryDashboard, PoolTogether, Zap, Wrap, Presale, V1Stake } from "./views";
...
function App() {
  ...
  const loadApp = useCallback(
    loadProvider => {
      ...
      console.log("tarzan===>loadApp:loadAppDetail");
      ...
      if (
        ...
        bonds.map(bond => {
          dispatch(
            calcBondDetails({ bond, value: "", provider: loadProvider, networkID: networkId, address: address }),
          );
        });
        lpTokens.map(lpToken => {
          dispatch(calcTokenDetails({ lpToken, provider: loadProvider, networkID: networkId, address: address }));
        });
      }
    },
    [networkId],
